...
async function verifyRoadmapBooks(roadmap: RoadmapResponse['roadmap']) {
  ...
  const verifiedNodes = verifiedCandidates.filter(
    (node): node is { id: string; title: string; author: string; position: { x: number; y: number } } =>
      node !== null
  )
          ...
          content: [
            {
              ...
              text: [
                ...
                'Each node MUST include non-empty id, title, author, and position.',
                ...
                'Return ONLY valid JSON with this shape: {"roadmap":{"title":string,"description":string,"hasBranches":boolean,"branchInfo":{"branchPoint":string,"tracks":[{"id":string,"name":string,"description":string}]},"nodes":[{"id":string,"title":string,"author":string,"branch":string,"requiresChoice":boolean,"position":{"x":number,"y":number}}],"edges":[{"source":string,"target":string}]}}',
              ].join(' '),
            },
          ],
        },
          ...
          content: [
            {
              ...
              text: [
                ...
                '각 노드는 반드시 id, title, author, position(x,y)을 포함해줘.',
                retryHint,
              ].join('\n'),
            },
          ],
